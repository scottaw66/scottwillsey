  ...
  return (tree) => {
    visit(tree, "text", (node) => {
      ...
      if (matches) {
        ...
        node.type = "html"; // Ensure the node type is HTML for correct rendering
        node.value = replacementTemplate(videoId);
      }
    });
  };
}

export function remarkMastodonLinks() {
  return (tree) => {
    visit(tree, "text", (node) => {
      const mastodonRegex =
        /^<?https:\/\/([a-zA-Z0-9.-]+)\/(@[\w-]+\/\d{10,20})>?$/;
      const replacementTemplate = (domain, id) =>
      ...
      const matches = node.value.match(mastodonRegex);
      if (matches) {
        const domain = matches[1],
          id = matches[2];
        node.type = "html"; // Ensure the node type is HTML for correct rendering
        node.value = replacementTemplate(domain, id);
      }
    });
  };
}

export function remarkThreadsLinks() {
  return (tree) => {
    visit(tree, "text", (node) => {
      const threadsRegex =
        /^<?https:\/\/www\.threads\.net\/(@[\w.]+)\/post\/([A-Za-z0-9_]+)>?/;
      const replacementTemplate = (user, id) =>
...
<script async src="https://www.threads.net/embed.js"></script>
      ...
      const matches = node.value.match(threadsRegex);
      if (matches) {
        const user = matches[1],
          id = matches[2];
        node.type = "html"; // Ensure the node type is HTML for correct rendering
        node.value = replacementTemplate(user, id);
      }
    });
  };
}
