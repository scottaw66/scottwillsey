...
2. **This branch is fully Zola at the repo root.** The Astro tree was frozen
   ...
   deleted. (Merged to `main` and deployed 2026-08-08 — `astro/…` paths in
   the phase checklists below are historical.)
...
- `zola init` now writes `zola.toml`; this repo used `config.toml` (the still-
  supported fallback) until 2026-08-09, then renamed to match the docs.
...
- [x] Install Zola (0.23.2); `zola.toml` (named `config.toml` until
      2026-08-09): `base_url`, `output_dir = "dist"`,
